...
int loadVideoList(char * splashFN) {
  ...
  while (infile.openNext(&rootDir, O_RDONLY) && aviCount < maxVideos) {
    ...
    if (fileName[0] != '.') {
      if ( !strcasecmp(fileName, "splash.avi") || !strcasecmp(fileName, "splash.tsv")) {
        // Do not add to list
        infile.getSFN(splashFN, 15);
      }else if (!strcasecmp(fileName + strlen(fileName) - 4, ".avi") || !strcasecmp(fileName + strlen(fileName) - 4, ".tsv")) {
        // Regular video
